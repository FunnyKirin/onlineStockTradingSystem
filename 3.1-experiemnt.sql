...
*/
DELIMITER //
CREATE PROCEDURE deleteEmployeeHourlyRate(IN ID INTEGER, HourlyRate INTEGER)
  #IF (ID > 0 AND ID < 1000000000) THEN
	 BEGIN
		 UPDATE Stock
...
CREATE PROCEDURE mostRevenue_CustomerRepresentative()
 BEGIN
	 ...
	 FROM Employee E
	 INNER JOIN
	    (SELECT E2.ID, MAX(Stock.PricePerShare * hasStock.NumShares) AS MaxRevenue
	    FROM Employee E2, Stock S2, hasStock
	    GROUP BY E2.ID) groupedE
...
CREATE PROCEDURE customer_mostRevenue()
 BEGIN
	...
	FROM Client C
	INNER JOIN
	    (SELECT C2.ID, MAX(Stock.PricePerShare * hasStock.NumShares) AS MaxRevenue
	    FROM Client C2, Stock S2, hasStock
	    GROUP BY C2.ID) groupedC
	ON C.ID = groupedC.ID 
