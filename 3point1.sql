...
CREATE PROCEDURE revenueByCustomerID(IN ID INTEGER)
 BEGIN
	 SELECT C.ID, S.StockSymbol AS 'Symbol', O.NumShares AS '# of Shares',
		(R.PricePerShare - S.PricePerShare) * (O.NumShares + H.NumShares) AS 'Revenue'
	 FROM Tranaction R, Trade T, Account A, Client C, StockOrder O, Stock S, HasStock H
     WHERE T.TransactionId = R.ID
		AND A.AccountId = T.AccountId
        AND C.ID = A.ClientID
        AND O.ID = T.OrderId
        AND HasStock.AccountId = A.ID
        AND T.StockId = S.StockSymbol;
 END //
