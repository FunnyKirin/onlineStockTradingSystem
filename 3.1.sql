...
DELIMITER //
CREATE PROCEDURE revenueByCustomer(IN ID INTEGER)
 BEGIN
	 SELECT hasStock.NumShares * Stock.PricePerShare
...
DELIMITER //
CREATE PROCEDURE revenueByCustomerRepresentative(IN Type CHAR(20))
 BEGIN
	 SELECT MAX()
	 FROM Trade, Stock
	 WHERE Stock.Type = Type;
...
DELIMITER //
CREATE PROCEDURE customer_mostRevenue(IN Type CHAR(20))
 BEGIN
	 SELECT* 
	 FROM Trade, Stock
	 WHERE Stock.Type = Type;
