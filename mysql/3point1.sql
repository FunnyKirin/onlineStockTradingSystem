...
DELIMITER //
CREATE PROCEDURE revenueByStock(IN Symbol CHAR(20))
BEGIN
	SELECT S.*, SUM((R.PricePerShare - S.PricePerShare) * O.NumShares) AS 'Max Revenue'
...
DELIMITER //
CREATE PROCEDURE revenueByCustomerID(IN CID INTEGER)
 BEGIN
	 SELECT C.ID, S.StockSymbol AS 'Symbol', O.NumShares AS '# of Shares',
		(R.PricePerShare - S.PricePerShare) * O.NumShares AS 'Revenue'
	 FROM Transaction R, Trade T, Account A, Client C, StockOrder O, Stock S
